...
def read_opt_inputs(fname = "kconv_input"):
    ...
    for input in inputs:
        ...
        if "econv" in key:
            econv = [v.strip() for v in val.strip().split(",")]
        if "pseudo" in key:
            pseudoset = val.strip()
        if "structure" in key:
            structure = val.strip()
...
def plot_econv(root, econv):
    nrgs = [get_nrg(opj(root, e))*Hartree for e in econv]
    es = [float(e) for e in econv]
    idcs = np.argsort(es)
    if len(es) > 3:
        ...
        for i in range(2):
            ...
            ax[i].plot([es[i] for i in idcs], [nrgs[i] for i in idcs])
            ax[i].scatter([es[i] for i in idcs], [nrgs[i] for i in idcs])
        ...
        ax[1].set_xticks(range(len(es)), [econv[i] for i in idcs])
    else:
        plt.plot(range(len(econv)), [nrgs[i] for i in idcs])
        plt.scatter(range(len(econv)), [nrgs[i] for i in idcs])
        plt.xticks(range(len(econv)), [econv[i] for i in idcs])
        plt.savefig(opj(root, "econv.png"))
...
def main():
    _work_dir, structure, fmax, max_steps, gpu, restart, pbc, lat_iters, use_jdft, freeze_base, freeze_tol, ortho, save_state, pseudoSet, econv = read_opt_inputs()
    os.chdir(_work_dir)
    opt_log = get_log_fn(_work_dir, "econv", False, restart=restart)
    exe_cmd = get_exe_cmd(gpu, opt_log)
    for e in econv:
        work_dir = opj(_work_dir, e)
        if not ope(work_dir):
            mkdir(work_dir)
