...
def setup_neb(
    ...
    fire_pickle_path = opj(neb_dir, "_FIRE.pckl")
    trajpath = opj(neb_dir, "neb.traj")
    # if ope(trajpath):
    #     traj = Trajectory(trajpath, 'r')
    #     neb.set_images(traj[-len(atoms_list):])
    # else:
    tmode = 'a' if Path(opj(neb_dir, "neb.traj")).exists() else 'w'
    #ftmode = 'a' if Path(opj(neb_dir, "fire.traj")).exists() else 'w'
    #tmode = "wa"
    traj = Trajectory(opj(neb_dir, "neb.traj"), tmode, neb, properties=['energy', 'forces'])
    #ftraj = Trajectory(opj(neb_dir, "fire.traj"), ftmode, neb, properties=['energy', 'forces'])
    dyn = FIRE(neb, logfile=logfile, restart=opj(neb_dir, "hessian.pckl"), trajectory=opj(neb_dir, "fire.traj"),)
    # if ope(fire_pickle_path):
    #     _dyn: dict = pickle.load(open(fire_pickle_path, "rb"))
    #     # dyn.v = _dyn["v"]
    #     # dyn.dt = _dyn["dt"]
    #     dyn.a = _dyn["a"]
        # dyn.e_last = _dyn["e_last"] if "e_last" in _dyn else None
        # dyn.r_last = _dyn["r_last"] if "r_last" in _dyn else None
        # dyn.v_last = _dyn["v_last"] if "v_last" in _dyn else None
    # if ope(fire_pickle_path):
    #     try:
    #         _dyn: dict = pickle.load(open(fire_pickle_path, "rb"))
    #         dyn.v = _dyn["v"]
    ...
    #         print(f"Error loading FIRE state: {e}")
    #         pass
    tmode = 'a' if Path(opj(neb_dir, "neb.traj")).exists() else 'w'
    traj = Trajectory(opj(neb_dir, "neb.traj"), tmode, neb, properties=['energy', 'forces'])
    dyn.attach(traj.write, interval=1)
    #dyn.attach(pickle.dump(dyn, open(fire_pickle_path, "wb")), interval=1)
    dyn.observers.append((pickle.dump, 1, ({
        # "v": dyn.v,
        # "dt": dyn.dt,
        "a": dyn.a,
        # "e_last": dyn.e_last if hasattr(dyn, "e_last") else None,
        # "r_last": dyn.r_last if hasattr(dyn, "r_last") else None,
        # "v_last": dyn.v_last if hasattr(dyn, "v_last") else None,
    }, open(fire_pickle_path, "wb")), {}))
    # dyn.attach(pickle.dump, interval=1, dyn, open(fire_pickle_path, "wb"), )
...
def main(debug=False):
    ...
    get_arb_calc = lambda root, cmds: _get_calc_new(exe_cmd, cmds, root, pseudoSet=pseudoSet, debug=debug, log_fn=print)
