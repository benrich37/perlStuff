...
from helpers.logx_helpers import write_scan_logx
...
if __name__ == '__main__':
    ...
    get_ionopt_calc = lambda root, nMax: _get_calc(exe_cmd, get_ionic_opt_cmds(cmds, nMax), root, JDFTx, debug=False, log_fn=se_log)
    get_latopt_calc = lambda root, nMax: _get_calc(exe_cmd, get_lattice_cmds(cmds, nMax, pbc=pbc), root, JDFTx, debug=False, log_fn=se_log)
    ####################################################################################################################
    if not skip_to_neb:
