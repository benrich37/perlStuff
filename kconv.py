...
def read_opt_inputs(fname = "kconv_input"):
    ...
    for input in inputs:
        ...
        if "kconv" in key:
            kconv = [v.strip() for v in val.strip().split(",")]
        if "pseudo" in key:
            pseudoset = val.strip()
        if "structure" in key:
            structure = val.strip()
    ...
    return work_dir, structure, fmax, max_steps, gpu, restart, pbc, lat_iters, use_jdft, freeze_base, freeze_tol, ortho, save_state, pseudoset, kconv
...
def main():
    _work_dir, structure, fmax, max_steps, gpu, restart, pbc, lat_iters, use_jdft, freeze_base, freeze_tol, ortho, save_state, pseudoSet, kconv = read_opt_inputs()
    os.chdir(_work_dir)
    opt_log = get_log_fn(_work_dir, "kconv", False, restart=restart)
    exe_cmd = get_exe_cmd(gpu, opt_log)
    for ks in kconv:
        work_dir = opj(_work_dir, ks)
        if not ope(work_dir):
            mkdir(work_dir)
