...
if __name__ == '__main__':
    ...
    if not debug:
        ...
        get_calc = lambda root: _get_calc(exe_cmd, cmds, root, debug=debug, log_fn=log_stuff)
    else:
        exe_cmd = " "
        from ase.calculators.emt import EMT as debug_calc

        get_calc = lambda root: _get_calc(exe_cmd, cmds, root, debug=debug, debug_fn=debug_calc, log_fn=log_stuff)
    if read_int_dirs:
